...
- 最后注明'本建议仅供参考'`
      ...
      const assistantResponse = formatResponse(data.choices[0].message.content);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="prose max-w-none">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6 h-[500px] overflow-y-auto flex flex-col">
          {conversation.map((item, index) => (
            <div
              key={index}
              className={`mb-4 p-4 rounded-lg ${item.role === 'user' ? 'ml-auto bg-blue-50 max-w-3/4 self-end' : 'mr-auto bg-gray-100 w-3/4'}`}
            >
              <p className="font-semibold text-sm text-gray-500">
                {item.role === 'user' ? '您' : '小助手'}
              </p>
