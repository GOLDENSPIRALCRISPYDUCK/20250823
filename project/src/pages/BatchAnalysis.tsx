...
const BatchAnalysis = () => {
  ...
  const handleExportFullReport = async () => {
    try {
      ...
      alert('导出完整报告时出错: ' + (error as Error).message);

      // Ensure loading indicator is removed
      const loadingModal = document.querySelector('div[style*="position: fixed; top: 0"]');
