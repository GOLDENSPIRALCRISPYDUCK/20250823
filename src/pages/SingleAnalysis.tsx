...
import { Upload, Activity, RefreshCw, Loader2, Download } from 'lucide-react';
...
const SingleAnalysis = () => {
    ...
    const [leftEyeFileName, setLeftEyeFileName] = useState<string | null>(null);
    const [rightEyeFileName, setRightEyeFileName] = useState<string | null>(null);
    ...
    const handleResetAll = () => {
        ...
        setLeftEyeFileName(null);
        setRightEyeFileName(null);
    ...
    const handleExportFullReport = async () => {
        try {
            // 获取文件名前缀（如 "0"）
            const getFilePrefix = (fileName: string | null) => {
                if (!fileName) return null;
                return fileName.split('_')[0];
            };

            const leftPrefix = getFilePrefix(leftEyeFileName);
            const rightPrefix = getFilePrefix(rightEyeFileName);

            if (!leftPrefix || !rightPrefix) {
                console.error('无法确定文件名前缀');
                alert('请先上传左右眼图片');
                return;
            }
            ...
            // 检查两个文件的前缀是否一致
            if (leftPrefix !== rightPrefix) {
                console.error('左右眼文件名前缀不匹配');
                alert('左右眼图片文件名前缀不一致，请确保上传同一患者的图片');
                return;
            }

            // 构建 PDF 文件路径
            const pdfPath = `/store/${leftPrefix}_report.pdf`;
    ...
    return (
        <div id="full-report-container" className="max-w-7xl mx-auto px-4 py-8">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold">单片影像分析</h1>
                <div className="flex gap-4">
                    <button
                        onClick={handleResetAll}
                        className="px-4 py-2 bg-blue-50 text-blue-600 rounded-lg flex items-center gap-2 hover:bg-blue-100 transition-colors shadow-md"
                    >
